    ...
    def upsert_recent_candles(self):
        ...
        try:
            ...
            if not df.empty:
                ...
                print(f"Đã upsert {len(df)} nến gần nhất theo datetime")
        except Exception as e:
            print(f"Lỗi trong upsert_recent_candles: {e}")
            import traceback

            traceback.print_exc()

    def run_realtime(self):
        print("Bắt đầu khởi tạo pipeline thời gian thực...")

        print("Bắt đầu backfill 5000 nến lịch sử")
        metatrader_data = self.extractor.fill_historical_data(n_candles=5000)
        self.loader.upsert_current_minute_candle(
            metatrader_data
        )  # Backfill using upsert logic

        # Lên lịch: upsert 10 nến gần nhất mỗi 10 giây
        interval = GOLD_DATA_CONFIG.get("realtime_interval_seconds", 10)
        print(f"Thiết lập lịch cho các thao tác upsert mỗi {interval} giây...")
        schedule.every(interval).seconds.do(self.upsert_recent_candles)

        print("Pipeline thời gian thực đã bắt đầu:")
        print(f"- Mỗi {interval} giây: Upsert 10 nến gần nhất")
        print("Nhấn Ctrl+C để dừng.")

        print("Pipeline thời gian thực đã bắt đầu thành công")
        print("DEBUG: Hoàn thành ghi log pipeline thời gian thực")
            ...
            while True:
                ...
                try:
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        ...
                        schedule.every(interval).seconds.do(self.upsert_recent_candles)
                        print("Pipeline đã khởi động lại sau lỗi")
                    else:
